...
export default function InventoryScreen() {
    ...
    // Remove all deliveries for a given item name+unit (used from Roommates/Balances tabs)
    const removeByGroup = (itemName: string, itemUnit: string) => {
        const matches = roomItems.filter(item => item.name.toLowerCase() === itemName.toLowerCase() && item.unit === itemUnit && item.quantity > 0);
        if (matches.length === 0) return;
        Alert.alert(
            `Remove "${itemName}"?`,
            matches.length === 1
                ? 'This delivery will be removed and balances will be recalculated.'
                : `All ${matches.length} deliveries of ${itemName} will be removed and balances recalculated.`,
            [
                { text: 'Cancel', style: 'cancel' },
                {
                    ...
                    onPress: async () => {
                        try {
                            await Promise.all(matches.map(item => deleteItem(item._id)));
                        } catch (err) {
                            Alert.alert('Could not remove', err instanceof Error ? err.message : 'Please try again.');
                        }
                    },
    ...
    <ScrollView contentContainerStyle={s.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor={colors.primary}/>}>
      ...
      {tab === 'Roommates' && members.map(member => {
          ...
          {rows.length > 0 && <View style={s.tableHeader}><Text style={[s.caption, s.flex]}>ITEM</Text><Text style={s.column}>BROUGHT</Text><Text style={s.column}>STILL OWES</Text><Text style={s.columnAction}></Text></View>}
          {rows.map(({ group, person }) => <TouchableOpacity key={group.key} style={s.tableRow} activeOpacity={0.7} onLongPress={() => removeByGroup(group.item.name, group.unit)} delayLongPress={500}><Text style={[s.itemName, s.flex]}>{group.item.name}{group.pending ? ' *' : ''}</Text><Text style={s.columnValue}>{amount(person!.brought, group.unit)}</Text><Text style={[s.columnValue, { color: person!.remaining ? colors.warningText : colors.successText }]}>{person!.remaining ? amount(person!.remaining, group.unit) : 'Settled'}</Text><TouchableOpacity style={s.deleteBtn} onPress={() => removeByGroup(group.item.name, group.unit)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}><Ionicons name="trash-outline" size={15} color={colors.dangerText} /></TouchableOpacity></TouchableOpacity>)}
      ...
      {tab === 'Balances' && groups.map(group => <View key={group.key} style={s.card}>
        <View style={s.row}>
          <View style={s.flex}><Text style={s.heading}>{group.item.name}</Text><Text style={s.muted}>{amount(group.total, group.unit)} brought together</Text><Text style={s.note}>Target: {amount(group.target, group.unit)} per roommate{group.pending ? ' · Provisional' : ''}</Text></View>
          <TouchableOpacity onPress={() => removeByGroup(group.item.name, group.unit)} style={s.deleteIconBtn} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}><Ionicons name="trash-outline" size={18} color={colors.dangerText} /></TouchableOpacity>
        </View>
      ...
      {tab === 'Verification' && <>
        ...
        {[...roomItems].filter(item => item.quantity > 0).sort((a, b) => Number(b.status === 'PENDING') - Number(a.status === 'PENDING') || b.createdAt.localeCompare(a.createdAt)).map(item => {
            ...
            <View style={s.actions}>{own ? <><TouchableOpacity onPress={() => openEdit(item)}><Text style={s.link}>Edit</Text></TouchableOpacity><TouchableOpacity onPress={() => { Alert.alert('Remove delivery?', 'This will recalculate item balances.', [{ text: 'Cancel', style: 'cancel' }, { text: 'Remove', style: 'destructive', onPress: async () => { try { await deleteItem(item._id); } catch (err) { Alert.alert('Could not remove', err instanceof Error ? err.message : 'Please try again.'); } } }]); }}><Text style={s.error}>Remove</Text></TouchableOpacity></> : item.status === 'PENDING' ? <><TouchableOpacity disabled={!!reviewing} onPress={() => review(item, false)}><Text style={s.error}>Reject</Text></TouchableOpacity><TouchableOpacity disabled={!!reviewing} onPress={() => review(item, true)} style={s.primary}><Text style={s.primaryText}>{reviewing === item._id ? 'Saving…' : 'Approve'}</Text></TouchableOpacity></> : null}</View>
          </View>;
            })}
...
const styles = (c: ThemeColors) => StyleSheet.create({
    ...
    deleteBtn: { width: 24, alignItems: 'center', justifyContent: 'center' },
    deleteIconBtn: { padding: 6, borderRadius: 8, backgroundColor: c.surfaceSubtle },
});
